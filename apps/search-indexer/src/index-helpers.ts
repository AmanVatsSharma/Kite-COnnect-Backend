 ...
 */
const WEEKLY_DOW_BY_UNDERLYING: Array<[RegExp, number]> = [
 ...
 */
export function weeklyDowForUnderlying(
 ...
 */
export function lastDowOfMonth(
  year: number,
  month: number,
  dow: number,
): number {
  ...
  for (let d = lastDay; d >= 1; d--) {
    if (new Date(year, month - 1, d).getDay() === dow) return d;
  }
  return lastDay;
}
