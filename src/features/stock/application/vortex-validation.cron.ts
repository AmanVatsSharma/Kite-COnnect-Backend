...
export class VortexValidationCronService {
  ...
  constructor(
    ...
    this.enabled =
      String(this.config.get('VALIDATION_CRON_ENABLED', 'true')) === 'true';
  }
  ...
  async nightlyValidation() {
    ...
    try {
      const exchanges: Array<'NSE_EQ' | 'NSE_FO' | 'NSE_CUR' | 'MCX_FO'> = [
        'NSE_EQ',
        'NSE_FO',
        'NSE_CUR',
        'MCX_FO',
      ...
      for (const ex of exchanges) {
        ...
        console.log(
          `[VortexValidationCron] Running nightly dry-run validation for ${ex}`,
        );
      ...
      this.logger.error(
        '[VortexValidationCron] Nightly validation failed',
        e as any,
      );
    }
  }
