...
export class StockService {
  ...
  constructor(
    ...
    @Inject(forwardRef(() => RequestBatchingService))
      ...
      for (const kiteInstrument of instruments) {
        ...
        if (existingInstrument) {
          ...
          await this.instrumentRepository.update(
            ...
            {
              exchange_token: Number(kiteInstrument.exchange_token) || null,
              tradingsymbol: kiteInstrument.tradingsymbol,
              name: kiteInstrument.name,
              last_price: Number(kiteInstrument.last_price) || 0,
              expiry: kiteInstrument.expiry,
              strike: Number(kiteInstrument.strike) || 0,
              tick_size: Number(kiteInstrument.tick_size) || 0.05,
              lot_size: Number(kiteInstrument.lot_size) || 1,
