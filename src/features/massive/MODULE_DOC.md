...
- **2026-04-26** — Multi-stream parallel WS: replaced single `MassiveWebSocketClient` with `MassiveMultiStreamClient` composite facade managing three parallel connections (stocks/forex/crypto). Added `classifyMassiveSymbol()` heuristic. Removed `realtime`/`assetClass` DB overrides (always realtime, always all 3). Fixed forex market name (`forex` → `fx`) in REST client.
- **2026-04-27** — REST polling fallback: when WS auth fails (plan does not include WebSocket), `MassiveMultiStreamClient` automatically switches to periodic REST snapshot polling for all three asset classes. `isWsConnected()` returns `true` during polling so the streaming layer treats the provider as live. `MASSIVE_POLL_INTERVAL_MS` env var (default 5000 ms) controls cadence. Added `getSubscribedTokens()` and `isAuthFailed()` to `MassiveWebSocketClient`.
