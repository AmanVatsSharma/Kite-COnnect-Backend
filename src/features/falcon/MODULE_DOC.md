...
- **2026-05-09** — Deep option chain endpoint:
  ...
  - `FalconController`: `GET /stock/falcon/options/chain/:symbol/deep` with optional `?expiry=YYYY-MM-DD` and `?strikes_around_atm=N` params.

- **2026-04-22** — Added `crossLinkProviderMappings()` to `FalconInstrumentService`: after each UIR upsert, runs two SQL UPDATE queries joining `instrument_mappings` on `instrument_token` to propagate `uir_id` bidirectionally between kite and vortex rows. This ensures the registry sees both provider tokens per UIR so that dual-subscribe and cross-provider tick fallback work correctly. Runs non-fatally inside the existing `try/catch` block and logs a warning on failure.
