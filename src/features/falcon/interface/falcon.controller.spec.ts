...
describe('FalconController', () => {
  ...
  const mockInstruments: Partial<FalconInstrumentService> = {
    getFalconInstruments: jest
      .fn()
      .mockResolvedValue({ instruments: [], total: 0 }),
    getFalconInstrumentStats: jest.fn().mockResolvedValue({
      total: 0,
      by_exchange: {},
      by_type: {},
      active: 0,
    ...
    syncFalconInstruments: jest
      .fn()
      .mockResolvedValue({ synced: 0, updated: 0 }),
    validateFalconInstruments: jest
      .fn()
      .mockResolvedValue({ tested: 0, invalid_instruments: [] }),
  };
  ...
  it('health should return success', async () => {
    (mockAdapter.getLTP as any).mockResolvedValueOnce({
      '26000': { last_price: 1 },
    });
