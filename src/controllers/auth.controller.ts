...
export class AuthController {
  ...
  @ApiResponse({
    status: 200,
    description: 'Falcon login URL',
    schema: {
      properties: {
        url: {
          type: 'string',
          example: 'https://kite.trade/connect/login?...',
        },
        state: { type: 'string' },
      },
    },
  ...
  async callback(
    ...
    try {
      ...
      throw new BadRequestException(
        `Falcon OAuth failed: ${e?.message || 'unknown error'}`,
      );
    }
...
export class VortexAuthController {
  ...
  @Get('login')
  @ApiOperation({
    summary: 'Get Vayu login URL',
    description:
      'Returns the Rupeezy Vayu OAuth login URL generated from your VORTEX_APP_ID.',
  })
  @ApiOkResponse({
    description: 'Vayu login URL',
    schema: {
      type: 'object',
      properties: {
        url: {
          type: 'string',
          example: 'https://flow.rupeezy.in?applicationId=YOUR_APP_ID',
        },
      },
    },
  })
  @ApiBadRequestResponse({
    ...
    schema: {
      ...
      properties: {
        ...
        message: {
          type: 'string',
          example: 'Vayu applicationId (VORTEX_APP_ID) not configured',
        },
  ...
  @Get('callback')
  @ApiOperation({
    summary: 'Vayu callback handler (exchanges auth->access_token)',
    description:
      'Handles redirect from Rupeezy Vayu. Reads the auth query param, computes checksum, calls Create Session API, and persists the access_token to the database.',
  })
  ...
  @ApiBadRequestResponse({
    description: 'Exchange failed or misconfigured',
    schema: {
      type: 'object',
      properties: {
        statusCode: { type: 'number', example: 400 },
        message: {
          type: 'string',
          example: 'Vayu session creation failed: ...',
        },
        error: { type: 'string', example: 'Bad Request' },
      },
    },
  ...
  async callback(@Query('auth') auth: string) {
    try {
      ...
      this.logger.log(
        `[Vayu] Configuration: appId=${appId ? 'present' : 'missing'}, apiKey=${apiKey ? 'present' : 'missing'}, baseUrl=${baseUrl}`,
      );

      if (!appId || !apiKey) {
        throw new BadRequestException(
          'Vayu envs missing: VORTEX_APP_ID, VORTEX_API_KEY',
        );
      }
      ...
      try {
        this.logger.log(
          `[Vayu] Creating session at ${createSessionUrl.replace(apiKey, '***')}`,
        );
        ...
        this.logger.log(
          `[Vayu] Session creation successful, received response`,
        );

        // Log full response for debugging
        this.logger.log(
          `[Vayu] Response structure: ${JSON.stringify({
        ...
        this.logger.error(
          `[Vayu] Session creation failed: ${typeof msg === 'string' ? msg : JSON.stringify(msg)} (status=${status || 'n/a'})`,
        );
        throw new BadRequestException(
          `Vayu session creation failed: ${typeof msg === 'string' ? msg : JSON.stringify(msg)} (status=${status || 'n/a'} url=${sanitizedUrl})`,
        );
      }
      ...
      if (sessionResp?.data?.status !== 'success') {
        this.logger.error(
          `[Vayu] Session creation returned non-success status: ${JSON.stringify(sessionResp?.data)}`,
        );
        throw new BadRequestException(
          `Vayu session creation failed: ${JSON.stringify(sessionResp?.data)}`,
        );
      }
      ...
      if (!accessToken) {
        this.logger.error(
          `[Vayu] Response data structure: ${JSON.stringify(sessionResp?.data)}`,
        );
        throw new BadRequestException(
          `Vayu session did not return access_token. Response: ${JSON.stringify(sessionResp?.data)}`,
        );
      }

      this.logger.log(
        `[Vayu] Access token extracted, length: ${accessToken.length}`,
      );
      ...
      try {
        ...
        if (parts.length >= 2) {
          ...
          if (payload?.exp) {
            ...
            ttl = remain;
            this.logger.log(
              `[Vayu] JWT TTL calculated: ${remain}s (expires at ${new Date(payload.exp * 1000).toISOString()})`,
            );
          }
        }
      } catch (e) {
        this.logger.warn(
          `[Vayu] Failed to parse JWT TTL, using fallback: ${ttl}s`,
        );
      }

      // Persist session in DB; deactivate previous
      this.logger.log(
        `[Vayu] Deactivating previous sessions and saving new session`,
      );
      await this.vortexSessionRepo
        .createQueryBuilder()
      ...
      const entity = this.vortexSessionRepo.create({
      ...
      try {
        if (
          typeof (this.vortexProvider as any).updateAccessToken === 'function'
        ) {
        ...
        this.logger.error(
          '[Vayu] Failed to update access token in provider',
          e as any,
        );
      }
      ...
      try {
        ...
        if (!status?.isStreaming) {
          ...
          this.logger.log(
            '[Vayu] Auto-started streaming after successful login',
          );
        } else {
          this.logger.log(
            '[Vayu] Streaming already active, reconnecting with new token',
          );
          if (
            typeof (this.streamService as any).reconnectIfStreaming ===
            'function'
          ) {
        ...
        this.logger.error(
          '[Vayu] Failed to start/reconnect streaming',
          e as any,
        );
      }
