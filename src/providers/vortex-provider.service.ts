...
class NoopTicker {
  ...
  connect() { this.logger.warn('[Vortex] NoopTicker.connect called (stream not configured)'); }
  disconnect() { this.logger.warn('[Vortex] NoopTicker.disconnect called'); }
  subscribe(_tokens: number[]) { this.logger.warn('[Vortex] NoopTicker.subscribe called'); }
  unsubscribe(_tokens: number[]) { this.logger.warn('[Vortex] NoopTicker.unsubscribe called'); }
  setMode(_mode: string, _tokens: number[]) { this.logger.warn('[Vortex] NoopTicker.setMode called'); }
}
...
export class VortexProviderService implements OnModuleInit, MarketDataProvider {
  ...
  constructor(private configService: ConfigService, @InjectRepository(VortexSession) private vortexSessionRepo: Repository<VortexSession>, @InjectRepository(Instrument) private instrumentRepo: Repository<Instrument>) {}
  ...
  async initialize(): Promise<void> {
    try {
      ...
      if (!apiKey || !baseUrl) {
        this.logger.warn('[Vortex] API key or base URL not configured. REST methods will return empty.');
      } else {
        this.http = axios.create({ baseURL: baseUrl, timeout: 10000, headers: { 'x-api-key': apiKey } });
  ...
  async getInstruments(exchange?: string, opts?: { csvUrl?: string }): Promise<any[]> {
    try {
      const csvUrl = opts?.csvUrl || this.configService.get('VORTEX_INSTRUMENTS_CSV_URL');
      if (!csvUrl) {
        this.logger.warn('[Vortex] CSV URL not configured for instruments. Returning empty list.');
        return [];
      }
      // Download with basic retry/backoff
      let attempt = 0; let res: Response | null = null; let lastErr: any = null;
      while (attempt < 3) {
        try {
          // Lazy import to avoid hard dependency if unused
          res = await fetch(csvUrl, { headers: { 'Accept-Encoding': 'gzip, deflate' } as any });
          if (res.ok) break;
      ...
      if (!res || !res.ok) {
        this.logger.warn(`[Vortex] Failed to download CSV from ${csvUrl}: ${lastErr?.message || 'unknown'}`);
        return [];
      }
      if (!res.ok) {
        this.logger.warn(`[Vortex] Failed to download CSV from ${csvUrl}: ${res.status}`);
        return [];
      }
      ...
      this.logger.error('[Vortex] Failed to fetch/parse instruments CSV', error as any);
      return [];
    }
  }
  ...
  async getQuote(tokens: string[]): Promise<Record<string, any>> {
    try {
      ...
      if (!this.http) {
        this.logger.warn('[Vortex] getQuote called without credentials. Returning empty result.');
        return {};
      }
  ...
  async getLTP(tokens: string[]): Promise<Record<string, any>> {
    try {
      await this.ensureTokenLoaded();
      if (!this.http) {
        this.logger.warn('[Vortex] getLTP called without credentials. Returning empty result.');
        return {};
      }
  ...
  async getOHLC(tokens: string[]): Promise<Record<string, any>> {
    try {
      await this.ensureTokenLoaded();
      if (!this.http) {
        this.logger.warn('[Vortex] getOHLC called without credentials. Returning empty result.');
        return {};
      }
  ...
  async getHistoricalData(token: number, from: string, to: string, interval: string): Promise<any> {
    try {
      await this.ensureTokenLoaded();
      if (!this.http) {
        this.logger.warn('[Vortex] getHistoricalData called without credentials. Returning empty candles.');
        return { candles: [] };
      }
      ...
      const candles: any[] = [];
      const len = Math.min(d.t?.length || 0, d.o?.length || 0, d.h?.length || 0, d.l?.length || 0, d.c?.length || 0);
      for (let i = 0; i < len; i++) {
      ...
      this.logger.error('[Vortex] getHistoricalData failed (non-fatal)', error as any);
      return { candles: [] };
    }
  }
  ...
  initializeTicker(): TickerLike {
    ...
    const streamUrl = this.configService.get('VORTEX_WS_URL') || 'wss://wire.rupeezy.in/ws';
    this.logger.log(`[Vortex] Using WebSocket URL: ${streamUrl.replace(/auth_token=[^&]*/, 'auth_token=***')}`);
    const self = this;
    class VortexTicker {
      ...
      private exchangeByToken: Map<number, 'NSE_EQ' | 'NSE_FO' | 'NSE_CUR' | 'MCX_FO'> = new Map();
      private pingTimer: NodeJS.Timeout | null = null;
      private lastPongAt: number = 0;

      on(event: string, fn: Function) { if (!this.handlers[event]) this.handlers[event] = []; this.handlers[event].push(fn); }
      emit(event: string, ...args: any[]) { (this.handlers[event]||[]).forEach(h=>{ try{ h(...args); }catch{} }); }

      connect() {
        const token = self.accessToken || self.configService.get('VORTEX_ACCESS_TOKEN');
        if (!token) { 
          self.logger.warn('[Vortex] No access_token for WS; did you login?'); 
          return; 
        }
        const url = `${streamUrl}?auth_token=${encodeURIComponent(token)}`;
        self.logger.log(`[Vortex] WS connecting to ${url.replace(token, '***')}`);
        ...
        this.ws.on('open', () => {
          ...
          if (this.subscribed.size > 0) {
            self.logger.log(`[Vortex] Resubscribing to ${this.subscribed.size} previously subscribed tokens`);
            this.resubscribeAll();
          }
          this.emit('connect');
        });
        this.ws.on('close', (code, reason) => {
          ...
          self.logger.warn(`[Vortex] WS disconnected with code ${code}: ${reason}`);
          this.stopHeartbeat();
        ...
        this.ws.on('message', (data: any) => {
          if (typeof data === 'string') {
            try { 
              ...
              self.logger.warn('[Vortex] Failed to parse text message', e as any);
            }
          } else if (Buffer.isBuffer(data)) {
            try {
              const ticks = self.parseBinaryTicks(data);
              if (ticks.length) {
                self.logger.debug(`[Vortex] Parsed ${ticks.length} binary ticks`);
                this.emit('ticks', ticks);
              }
            } catch (e) {
              self.logger.error('[Vortex] Failed to parse binary tick packet', e as any);
            }
          }
        });
      }
      ...
      unsubscribe(tokens: number[]) {
        ...
        (async () => {
          try {
            ...
            if (missing.length) {
              const exMapRaw = await self.getExchangesForTokens(missing.map(t => String(t)));
              missing.forEach(t => {
                const ex = (exMapRaw.get(String(t)) || 'NSE_EQ') as 'NSE_EQ' | 'NSE_FO' | 'NSE_CUR' | 'MCX_FO';
                this.exchangeByToken.set(t, ex);
              });
            ...
            self.logger.warn('[Vortex] unsubscribe exchange resolution failed; using NSE_EQ fallback', e as any);
          }
          tokens.forEach(t => {
            const ex = this.exchangeByToken.get(t) || 'NSE_EQ';
            const mode = this.modeByToken.get(t) || 'ltp';
            this.send({ exchange: ex, token: t, mode, message_type: 'unsubscribe' });
            this.subscribed.delete(t);
            this.modeByToken.delete(t);
            this.exchangeByToken.delete(t);
          });
          if (tokens.length) self.logger.log(`[Vortex] Unsubscribed ${tokens.length} tokens`);
        })();
      }
      setMode(mode: string, tokens: number[]) {
        ...
        (async () => {
          try {
            ...
            self.logger.warn('[Vortex] setMode exchange resolution failed; using NSE_EQ fallback', e as any);
          }
          target.forEach(t => {
            const ex = this.exchangeByToken.get(t) || 'NSE_EQ';
            this.send({ exchange: ex, token: t, mode: m, message_type: 'subscribe' });
          });
          if (target.length) self.logger.log(`[Vortex] Mode set to ${m} for ${target.length} tokens`);
        })();
      }
      ...
      private resubscribeAll() {
        ...
        (async () => {
          try {
            ...
            self.logger.warn('[Vortex] resubscribeAll exchange resolution failed; using NSE_EQ fallback', e as any);
          }
          for (const t of tokens) {
            const mode = this.modeByToken.get(t) || 'ltp';
            const ex = this.exchangeByToken.get(t) || 'NSE_EQ';
            this.send({ exchange: ex, token: t, mode, message_type: 'subscribe' });
          }
      ...
      if (buf.length >= 2) {
        ...
        if (possibleCount > 0 && possibleCount < 2000) {
          ...
          while (parsed < possibleCount && offset + 2 <= buf.length) {
            const size = buf.readUInt16LE(offset); offset += 2;
          ...
          if (parsed !== possibleCount) {
            this.logger.debug?.(`[Vortex] Header count=${possibleCount} parsed=${parsed} totalBytes=${buf.length}`);
          }
        }
      }
  ...
  private parseOneTick(payload: Buffer): any | null {
    try {
      ...
      const exchange = payload.subarray(0, 10).toString('ascii').replace(/\u0000/g, '').trim();
      const token = payload.readInt32LE(10);
      
      if (len === 22) {
        // LTP mode: 22 bytes
        const ltp = payload.readDoubleLE(14);
        this.logger.debug(`[Vortex] Parsed LTP tick: token=${token}, exchange=${exchange}, price=${ltp}`);
      ...
      if (len === 62) {
        ...
        this.logger.debug(`[Vortex] Parsed OHLCV tick: token=${token}, exchange=${exchange}, OHLC=${open}/${high}/${low}/${close}, volume=${volume}`);
        return { 
      ...
      if (len === 266) {
        ...
        this.logger.debug(`[Vortex] Parsed FULL tick: token=${token}, exchange=${exchange}, price=${ltp}, depth=${depth.buy.length}/${depth.sell.length} levels`);
        return { 
          instrument_token: token, 
          exchange, 
          last_price: ltp, 
      ...
      this.logger.warn(`[Vortex] Unknown tick length: ${len} bytes, expected 22/62/266`);
      return null;
    } catch (e) {
      this.logger.error(`[Vortex] parseOneTick failed for payload length ${payload.length}`, e as any);
      return null;
    }
  }
  ...
  private async getExchangesForTokens(tokens: string[]): Promise<Map<string, 'NSE_EQ' | 'NSE_FO' | 'NSE_CUR' | 'MCX_FO'>> {
    ...
    try {
      ...
      const rows = await this.instrumentRepo.find({ where: { instrument_token: In(nums) } as any, select: ['instrument_token', 'exchange', 'segment'] as any });
      for (const r of rows) {
      ...
      if (notFound.length > 0) {
        this.logger.debug(`[Vortex] Using NSE_EQ fallback for ${notFound.length} tokens not in DB: ${notFound.slice(0, 5).join(',')}`);
      }
    } catch (e) {
      this.logger.warn('[Vortex] getExchangesForTokens failed; all tokens will use NSE_EQ fallback', e as any);
    }
    return map;
  }

  private normalizeExchange(ex: string): 'NSE_EQ' | 'NSE_FO' | 'NSE_CUR' | 'MCX_FO' | null {
    const s = (ex || '').toUpperCase();
    if (s.includes('NSE_EQ') || s === 'NSE' || s === 'EQ' || s.includes('EQUITY')) return 'NSE_EQ';
    if (s.includes('NSE_FO') || s.includes('FO') || s.includes('FUT') || s.includes('FNO')) return 'NSE_FO';
    if (s.includes('NSE_CUR') || s.includes('CDS') || s.includes('CUR')) return 'NSE_CUR';
  ...
  private async ensureTokenLoaded(): Promise<void> {
    ...
    try {
      ...
      if (active?.access_token) {
        // If expired, skip
        if (active.expires_at && active.expires_at.getTime() <= Date.now()) {
          this.logger.warn('[Vortex] Active DB token is expired; please login again');
          return;
        }
        this.accessToken = active.access_token;
  ...
  async ping(): Promise<{ httpOk: boolean; reason?: string }> {
    try {
      ...
      try {
        const resp = await this.http.get(url, { headers: this.authHeaders(), timeout: 3000 });
        const ok = !!resp?.data;
        ...
        if (status === 401 || status === 403) {
          ...
          this.logger.warn(`[Vortex] Health ping failed: auth error (${status})`);
        ...
        this.logger.error(`[Vortex] Health ping failed: ${status ? `HTTP ${status}` : e?.code || 'network_error'}`);
        return { httpOk: false, reason: status ? `http_${status}` : (e?.code || 'network_error') };
      }
    } catch (e: any) {
      this.logger.error(`[Vortex] Health ping error: ${e?.message || 'unknown'}`);
